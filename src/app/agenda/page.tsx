...
import { Calendar, Clock, MapPin, User, Plus, CheckCircle, Trash2, Phone, MessageCircle } from "lucide-react";
...
export default function Agenda() {
  ...
  function refrescar() { setCitas(getCitas()); }
  ...
  function marcarCompletada(cita: Cita) {
    setResultadoModal({ ...cita });
  }

  function guardarResultado() {
    if (!resultadoModal) return;
    saveCita({ ...resultadoModal, completada: true });
    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">
            {pendientesHoy > 0
              ? `Tienes ${pendientesHoy} cita(s) hoy`
              : "Sin citas para hoy"}
          </p>
        </div>
        ...
        {(["todas", "hoy", "pendientes", "completadas"] as const).map((f) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors capitalize ${
              filtro === f ? "bg-cyan-600 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
            }`}
          >
      ...
      {citasFiltradas.length === 0 ? (
        <div className="text-center py-20 text-slate-400">
          ...
          <p className="text-sm mt-1">Crea una nueva cita con el botón de arriba</p>
        </div>
      ) : (
        <div className="space-y-3">
          {citasFiltradas.map((cita) => {
            ...
            return (
              <Card
                key={cita.id}
                className={`${cita.completada ? "opacity-60" : ""} ${esHoy && !cita.completada ? "ring-2 ring-purple-400" : ""}`}
              >
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <Badge variant="secondary" className="text-xs">
                          {cita.asignado_a === "dueno" ? "👤 Dueño" : "👥 Vendedor"}
                        </Badge>
                        {esHoy && !cita.completada && (
                          <span className="text-xs bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full font-medium">HOY</span>
                        )}
                      ...
                      <div className="flex flex-wrap gap-3 mt-2 text-xs text-slate-600">
                        ...
                        {cita.direccion && (
                          <span className="flex items-center gap-1 truncate"><MapPin size={12} /> {cita.direccion}</span>
                        )}
                      </div>

                      {cita.notas && (
                        <p className="text-xs text-slate-500 mt-1 italic">{cita.notas}</p>
                      )}

                      {/* Acciones rápidas si hay contacto */}
                      {contacto && !cita.completada && (
                        <div className="flex gap-2 mt-2">
                          {contacto.telefono && (
                            <a
                              href={`https://wa.me/52${contacto.telefono.replace(/\D/g, "")}?text=Hola!%20Le%20recordamos%20nuestra%20cita%20de%20hoy%20de%20NutriFreeze%20a%20las%20${cita.hora}`}
                              target="_blank"
      ...
      <Dialog open={modal} onOpenChange={setModal}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Nueva cita / visita</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => setForm({ ...form, contacto_id: v || undefined })}
              >
                <SelectTrigger>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Select
                  value={form.tipo ?? "visita"}
                  onValueChange={(v) => setForm({ ...form, tipo: v as Cita["tipo"] })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div>
                ...
                <Select
                  value={form.asignado_a ?? "dueno"}
                  onValueChange={(v) => setForm({ ...form, asignado_a: v as Cita["asignado_a"] })}
                >
            ...
            <div>
              ...
              <Input
                placeholder="Notas previas, qué le interesó..."
                value={form.notas ?? ""}
            ...
            <div className="flex gap-2 justify-end pt-1">
              ...
              <Button
                ...
                onClick={guardarCita}
                disabled={!form.contacto_id || !form.fecha || !form.hora}
              >
                Agendar cita
              </Button>
            </div>
      ...
      {/* Modal resultado de cita */}
      <Dialog open={!!resultadoModal} onOpenChange={() => setResultadoModal(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Resultado de la cita — {resultadoModal?.contacto_nombre}</DialogTitle>
          </DialogHeader>
          {resultadoModal && (
            <div className="space-y-3">
              <div>
                ...
                <Select
                  value={resultadoModal.resultado ?? "pendiente"}
                  onValueChange={(v) => setResultadoModal({ ...resultadoModal, resultado: v as Cita["resultado"] })}
                >
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="¿Qué pasó en la visita? ¿Qué le interesó? ¿Próximo paso?"
                  value={resultadoModal.notas ?? ""}
                  onChange={(e) => setResultadoModal({ ...resultadoModal, notas: e.target.value })}
                />
              </div>
